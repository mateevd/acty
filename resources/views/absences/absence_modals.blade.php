{{--MODAL DELETE --}}
<div class="modal" id="deleteAbsence" tabindex="-1" role="dialog" aria-labelledby="Supprimer une absence">
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			<div class="modal-custom-title">{{trans('app.delete')}}</div>
			<div class="modal-custom-libelle">{{trans('absences.lab_absence')}}</div>
			<div class="modal-custom-description">{{trans('absences.act_delete_confirm')}}</div>
			<div class="modal-custom-description show-warning-fatal">{{trans('app.nowayback')}}</div>

			<form action="{{route('absences.destroy')}}" method="post" class="hide-submit modal-custom-form">
				{{method_field('delete')}}
				{{csrf_field()}}

				<input type="hidden" name="absence_id" id="absence_id">

				<div class="modal-custom-detail style-ids" id="absence_id"></div>
...
{{--MODAL EDIT --}}
@if(isset($userAbsence))
	<div class="modal" id="editAbsence" tabindex="-1" role="dialog" aria-labelledby="Éditer une absence">
		<div class="modal-dialog modal-large" role="document">
			<div class="modal-content">
				<div class="modal-custom-title">Éditer</div>
				<div class="modal-custom-libelle">Absence</div>

				<form id="absence_update" action="{{route('absences.update', $userAbsence->absence_id)}}" method="post"
					...
					<div class="flex-row flex-wrap-yes justify-center items-center pad-v-small pad-h-medium bg-important-back width-100 ln-b-dashed-small">
						...
						<span class="pad-h-medium">
							{{ Form::date("absence_date", \Carbon\Carbon::parse($current_date)->startOfMonth(),["class"=>"form-control modal-custom-fields-input style-important", 'placeholder'=>trans('absences.lab_total_days'), 'required', 'tabindex'=>'1']) }}
					...
					<div class="flex-row flex-wrap-yes justify-evenly pad-h-medium pad-v-small pad-v-bottom-medium ln-b-solid-small width-100">
						<div class="flex-col flex-wrap-no justify-flex-end text-center pad-h-only-small">
							{{ Form::label("absence_type_id", trans('absences.lab_type'),["class"=>"dashboard-label bold"]) }}
							{{ Form::select("absence_type_id", $absence_types, null,["class"=>"form-control modal-custom-fields-input", 'tabindex'=>'2']) }}
						</div>
					</div>

					<div class="flex-row flex-wrap-yes pad-h-medium pad-v-small justify-center items-center ln-t-solid-small width-100">
						<label for="hours_history" class="dashboard-label text-center bold">Ajouter / Ôter des jours</label>
					</div>
					...
					<div class="flex-row pad-h-medium pad-v-small flex-wrap-no justify-center align-center">
						<div class="flex-row flex-wrap-yes justify-evenly pad-h-medium pad-v-small flex-wrap-no justify-center align-center" id="hoursBtns">
							...
							<input type="number" id="absence_days" name="absence_days"
							       class="form-control modal-custom-fields-input input-hours-absences" count="0"
							       value="0"
							       step="any" placeholder="{{trans('absences.lab_total_days')}}" required='required'
							       disabled>
						</div>
					...
					<div class="flex-row flex-wrap-yes justify-evenly pad-h-medium pad-v-small width-100">
						<label for="absence_description" class="dashboard-label bold">{{ucfirst(trans('app.description'))}}</label>
						<input type="textarea" class="form-control modal-custom-area-input" name="absence_description"
