...
class HomeController extends Controller
{
	...
	public function index()
	{
		...
		if (session()->has(['current_date', 'current_month', 'current_year'])) {
			$current_date = session()->get('current_date');
			$current_month = (int)session()->get('current_month');
			$current_year = (int)session()->get('current_year');
		} else {
			$current_date = Carbon::now();
		...
		$entity_activities_count_by_status = DB::table('activities')
			...
			->whereIn('activities.status', [$status_active, $status_terminated, $status_not_validated, $status_validated])
		...
		switch ($entity_activities_count_by_status) {
			case auth()->user()->roles->name == (config('constants.role_directeur')) || auth()->user()->roles->name == (config('constants.role_admin')):
				$entity_activities_count_by_status = $entity_activities_count_by_status->where('activities.department_id', '=', auth()->user()->department_id)->get();
				break;
			case (auth()->user()->roles->name == (config('constants.role_service'))):
				$entity_activities_count_by_status = $entity_activities_count_by_status->where('activities.service_id', '=', auth()->user()->service_id)->get();
				break;
			case (auth()->user()->roles->name == (config('constants.role_projet'))):
				$entity_activities_count_by_status = $entity_activities_count_by_status->where('activities.manager', '=', auth()->user()->id)->get();
				break;
			default:
				$entity_activities_count_by_status = $entity_activities_count_by_status->where('activities.manager', '=', auth()->user()->id)->get();
				break;
		...
		foreach ($entity_activities_count_by_status as $status_count) {
			if ($status_count->status == $status_active) $entity_activities_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $entity_activities_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_activities_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $entity_activities_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $entity_activities_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_activities_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $entity_activities_validated_count += $status_count->number;
			}
		}
		...
		foreach ($user_activities_count_by_status as $status_count) {
			if ($status_count->status == $status_active) $user_activities_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $user_activities_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $user_activities_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $user_activities_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $user_activities_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $user_activities_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $user_activities_validated_count += $status_count->number;
			}
		}
		...
		$entity_tasks_count_by_status = DB::table('tasks')
			...
			->whereIn('activities.status', [$status_active, $status_terminated, $status_not_validated, $status_validated])
			->whereIn('tasks.status', [$status_active, $status_terminated, $status_not_validated, $status_validated])
		...
		foreach ($entity_tasks_count_by_status as $status_count) {
			if ($status_count->status == $status_active) $entity_tasks_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $entity_tasks_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_tasks_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $entity_tasks_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $entity_tasks_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_tasks_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $entity_tasks_validated_count += $status_count->number;
			}
		}
		...
		foreach ($user_tasks_count_by_status as $status_count) {
			if ($status_count->status == $status_active) $user_tasks_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $user_tasks_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $user_tasks_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $user_tasks_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $user_tasks_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $user_tasks_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $user_tasks_validated_count += $status_count->number;
			}
		}
		...
		foreach ($entity_activities_count_by_status_and_entity as $status_count) {
			if ($status_count->status == $status_active) $entity_activities_for_entity_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $entity_activities_for_entity_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_activities_for_entity_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $entity_activities_for_entity_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $entity_activities_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_activities_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $entity_activities_for_entity_validated_count += $status_count->number;
			}
		...
		foreach ($user_activities_count_by_status_and_entity as $status_count) {
			if ($status_count->status == $status_active) $user_activities_for_entity_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $user_activities_for_entity_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $user_activities_for_entity_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $user_activities_for_entity_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $user_activities_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $user_activities_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $user_activities_for_entity_validated_count += $status_count->number;
			}
		...
		foreach ($entity_tasks_count_by_status_and_entity as $status_count) {
			if ($status_count->status == $status_active) $entity_tasks_for_entity_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $entity_tasks_for_entity_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_tasks_for_entity_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $entity_tasks_for_entity_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $entity_tasks_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $entity_tasks_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $entity_tasks_for_entity_validated_count += $status_count->number;
			}
		}
		...
		foreach ($user_tasks_count_by_status_and_entity as $status_count) {
			if ($status_count->status == $status_active) $user_tasks_for_entity_running_count = $status_count->number;
			
			if($cra_validate)
			{
				if ($status_count->status == $status_terminated) $user_tasks_for_entity_terminated_count = $status_count->number;
				if ($status_count->status == $status_not_validated) $user_tasks_for_entity_not_validated_count = $status_count->number;
				if ($status_count->status == $status_validated) $user_tasks_for_entity_validated_count = $status_count->number;
			}
			else
			{
				if ($status_count->status == $status_terminated) $user_tasks_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_not_validated) $user_tasks_for_entity_validated_count += $status_count->number;
				if ($status_count->status == $status_validated) $user_tasks_for_entity_validated_count += $status_count->number;
			}
		}
		...
		$entity_realise_by_month = DB::table('work_days')
			...
			->whereIn('work_days.status', [$status_active, $status_terminated, $status_not_validated, $status_validated])
		...
		$count_ppl = DB::table('users')
			->whereIn('users.status', [$status_active]);
